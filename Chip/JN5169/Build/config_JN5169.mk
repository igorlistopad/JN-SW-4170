...
CFLAGS += -DJN516x=5160
CFLAGS += -DJN5169=5169
CFLAGS += -DJENNIC_CHIP_NAME=_JN5169
...
ifneq ($(JENNIC_STACK), None)
    ifeq ($(JENNIC_MAC), MAC)
        LDLIBS += AppApi_$(REDUCED_MAC_LIB_SUFFIX)$(JENNIC_CHIP)
        LDLIBS += MAC_$(REDUCED_MAC_LIB_SUFFIX)$(JENNIC_CHIP)
        LDLIBS += TimerServer_$(JENNIC_CHIP_FAMILY)
        LDLIBS += TOF_$(JENNIC_CHIP_FAMILY)
        LDLIBS += Xcv_$(JENNIC_CHIP)
    else
        ifneq ($(findstring $(JENNIC_MAC),MiniMac MiniMacShim), )
            LDLIBS += MiniMac_$(MAC_IPN)$(JENNIC_CHIP)
            LDLIBS += MiniMacShim_$(JENNIC_CHIP_FAMILY)
        endif
    endif
    LDLIBS += MMAC_$(JENNIC_CHIP)
    LDLIBS += JPT_$(JENNIC_CHIP)
endif
LDLIBS += Aes_$(JENNIC_CHIP_FAMILY)
LDLIBS += HardwareApi_$(JENNIC_CHIP)
