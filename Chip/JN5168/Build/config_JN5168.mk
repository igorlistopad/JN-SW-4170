...
ifneq ($(JENNIC_STACK), None)
    ifeq ($(JENNIC_MAC), MAC)
        ...
        ifneq ($(findstring $(JENNIC_MAC),MiniMac MiniMacShim),)
            LDLIBS += MiniMac_$(MAC_IPN)$(JENNIC_CHIP_FAMILY)
...
ifneq ($(findstring $(JENNIC_MAC),MiniMac MiniMacShim), )
    ifeq ($(JENNIC_STACK), JIP)
        # Include 15.4-2006 security code
        LDFLAGS += -Wl,-ueSecurityTxPrepare -Wl,-ueSecurityTxEncrypt -Wl,-ubSecurityRxProcess
    endif
    ifeq ($(JENNIC_STACK), JENNET)
        # Include 15.4-2003 security code
        LDFLAGS += -Wl,-defsym,eSecurityTxPrepare=eSecurity2003TxPrepare
        LDFLAGS += -Wl,-defsym,eSecurityTxEncrypt=eSecurity2003TxEncrypt
        LDFLAGS += -Wl,-defsym,bSecurityRxProcess=bSecurity2003RxProcess
        LDFLAGS += -Wl,-ueSecurity2003TxPrepare -Wl,-ueSecurity2003TxEncrypt -Wl,-ubSecurity2003RxProcess
    endif
endif
